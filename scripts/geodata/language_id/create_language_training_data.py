...
from geodata.osm.osm_address_training_data import WAYS_LANGUAGE_DATA_FILENAME, ADDRESS_LANGUAGE_DATA_FILENAME, TOPONYM_LANGUAGE_DATA_FILENAME, VENUE_LANGUAGE_DATA_FILENAME
...
def create_language_training_data(osm_dir, split_data=True, train_split=0.8, cv_split=0.1):
    ...
    venues_path = os.path.join(osm_dir, VENUE_LANGUAGE_DATA_FILENAME)

    if os.system(' '.join(['cat', venues_path, '>>', language_all_path])) != 0:
        raise SystemError('Could not find {}'.format(venues_path))

    languages_random_path = os.path.join(osm_dir, LANGAUGES_RANDOM_FILE)

    if os.system(' '.join(['shuf', '--random-source=/dev/urandom', language_all_path, '>', languages_random_path])) != 0:
